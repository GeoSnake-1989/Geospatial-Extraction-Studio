...
10. a second clean-tree and exact `v0.4.6` tag check immediately before publication;
11. NSIS installer, SHA-256 checksum, matching tagged source archive, and
    source-bound installer provenance generation.

A normal build refuses to create `release/Geospatial-Extraction-Studio-Setup-0.4.6.exe` while any matched native component has a status other than `approved` in `native-components.json`. NSIS output is accepted only from the hash-pinned 3.12 `makensis.exe` using the
